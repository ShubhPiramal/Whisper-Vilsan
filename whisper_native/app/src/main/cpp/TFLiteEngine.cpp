...
int TFLiteEngine:: loadModel(const char *modelPath, const bool isMultilingual) {
    ...
    if (!g_whisper_tflite.is_whisper_tflite_initialized) {
       ...
       const char* vocabData = nullptr;
        ...
        // Try to initialize GPU delegate
        if (initializeGpuDelegate()) {
            std::cout << "GPU delegate initialized successfully" << std::endl;
            if (g_whisper_tflite.interpreter->ModifyGraphWithDelegate(gpu_delegate_) != kTfLiteOk) {
                std::cout << "Failed to apply GPU delegate, falling back to CPU" << std::endl;
                cleanupGpuDelegate();
            } else {
                std::cout << "GPU acceleration enabled" << std::endl;
            }
        } else {
            std::cout << "GPU delegate initialization failed, using CPU" << std::endl;
        }
...
std::string TFLiteEngine::transcribeBuffer(std::vector<float> samples) {
    ...
    // Run inference
    // Only set CPU threads if GPU delegate is not being used
...
bool TFLiteEngine::initializeGpuDelegate() {
    try {
        ...
        return gpu_delegate_ != nullptr;
    } catch (...) {
        std::cout << "Exception occurred during GPU delegate initialization" << std::endl;
        return false;
    }
}

void TFLiteEngine::cleanupGpuDelegate() {
    if (gpu_delegate_ != nullptr) {
